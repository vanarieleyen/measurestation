...
BEGIN
  ...
  D.dFile.Seek(D.HeadLen+(RecNum-1)*D.RecLen, soFromBeginning);
  dbfError := IOResult;
  IF dbfError = 0 THEN
  BEGIN
    //BlockRead(D.dFile, D.CurRecord^, D.RecLen, Result);
    D.dFile.ReadBuffer(D.CurRecord^, D.RecLen);
    Result := D.RecLen;
    dbfError := IOResult;
...
BEGIN
  ...
  BEGIN
    ...
    D.dFile.WriteBuffer(D.CurRecord^, D.RecLen);
    dbfError := IOResult;
  END;
...
CONST EofMark : Byte = $1A;
  ...
  BEGIN
    ...
    END;
    //seek(D.dFile, 0);
    D.dFile.Seek(0, soFromBeginning);
    dbfError := IOResult;
    IF dbfError = 0 THEN
    BEGIN
      //BlockWrite(D.dFile, D.HeadProlog, 8);
      D.dFile.WriteBuffer(D.HeadProlog, 8);
      dbfError := IOResult;
    END;
    dbfOK := (dbfError = 0);
  END;
...
BEGIN                       { CloseDbf }
  ...
  BEGIN
    ...
    BEGIN
      ...
      D.dFile.Seek( D.HeadLen+D.NumRecs*D.RecLen, soFromBeginning);
      dbfError := IOResult;
    END;
    IF dbfError = 0 THEN
    BEGIN
      //BlockWrite(D.dFile, EofMark, 1); {Put EOF marker }
      D.dFile.WriteBuffer(EofMark, 1);
      dbfError := IOResult;
    END;
  END;   { IF Updated }
  IF dbfError = 0 THEN
...
//    BlockRead(D.dFile, H, MAX_HEADER, Result);
    D.dFile.ReadBuffer(H, MAX_HEADER);
    Result := MAX_HEADER;
    dbfError := IOResult;
...
BEGIN                       { OpenDbf }
  ...
  if FileExists(D.FileName) then begin
    D.dFile := TFileStream.Create(D.FileName, fmOpenReadWrite or fmShareDenyNone);
...
BEGIN            { CreateDbf }
  ...
  D.FileName := fn;
  //Assign(D.dFile, D.FileName);
  D.dFile := TFileStream.Create(D.FileName, fmCreate or fmOpenReadWrite or fmShareDenyNone);
  //Rewrite(D.dFile, 1); {Will overwrite if file exists!}
  dbfError := IOResult;
  IF dbfError = 0 THEN
  BEGIN
    //BlockWrite(D.dFile,tHeader,Succ(D.HeadLen));
    D.dFile.WriteBuffer(tHeader,Succ(D.HeadLen));
    dbfError := IOResult;
  END;
  IF dbfError = 0 THEN
  BEGIN
