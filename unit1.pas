...
uses
  ...
  ExtCtrls, Buttons, Grids, Math, dbhi;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure StringGrid3MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
  ...
  public
    ...
    procedure Action(line:string; msg: string);
  end;
...
begin
  ...
  // get the height of Memo1 in lines
  tmpCanvas := TControlCanvas.Create;
  tmpCanvas.Control  := Memo1;
  tmpCanvas.Font.Assign(Memo1.Font);
  h := tmpCanvas.TextHeight('qQpP|]}');
  lines := round(Memo1.Height/h)-1;
  tmpCanvas.Free;
end;
...
procedure TForm1.Action(line: string; msg: string);
begin
  Memo1.Append('('+line+') '+msg);
  if (memo1.Lines.Count > lines) then
...
begin
  ...
  // show the details
  if ctdata2.Locate('YPBH', YPBH) then
  begin
    StringGrid1.RowCount := Length(ctdata2.filter) + 1;
    for r := 1 to Length(ctdata2.filter) do
    begin
      for i := 0 to ctdata2.NumFields - 1 do
...
var
  sticks: string;
  YPBH, nr, MeasFormat, row: string;
  fs: int64;
  recordnr, detailnr: integer;
  spec_hd, spec_circ, spec_len, spec_pd, spec_vent, spec_weight: string;
  n1, n2: single;
  spec_name, spec_weight_min, spec_weight_max, spec_circ_min, spec_circ_max, spec_len_min, spec_len_max: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      Open;
      if (FieldByName('force_update').AsInteger > 0) then
      begin
        ...
        begin
          ...
          if (FieldByName('weight').AsString = '') then
            spec_weight := '0'
          else
            spec_weight := FieldByName('weight').AsString;
          if (FieldByName('weight_min').AsString = '') then
            spec_weight_min := '0'
          else
            spec_weight_min := FieldByName('weight_min').AsString;
          if (FieldByName('weight_max').AsString = '') then
            spec_weight_max := '0'
          else
            spec_weight_max := FieldByName('weight_max').AsString;
          if (FieldByName('circ').AsString = '') then
            spec_circ := '0'
          else
            spec_circ := FieldByName('circ').AsString;
          if (FieldByName('circ_min').AsString = '') then
            spec_circ_min := '0'
          else
            spec_circ_min := FieldByName('circ_min').AsString;
          if (FieldByName('circ_max').AsString = '') then
            spec_circ_max := '0'
          else
            spec_circ_max := FieldByName('circ_max').AsString;
          if (FieldByName('len').AsString = '') then
            spec_len := '0'
          else
            spec_len := FieldByName('len').AsString;
          if (FieldByName('len_min').AsString = '') then
            spec_len_min := '0'
          else
            spec_len_min := FieldByName('len_min').AsString;
          if (FieldByName('len_max').AsString = '') then
            spec_len_max := '0'
          else
            spec_len_max := FieldByName('len_max').AsString;
          if (FieldByName('pd').AsString = '') then
            spec_pd := '0'
          else
            spec_pd := FieldByName('pd').AsString;
          if (FieldByName('pd_min').AsString = '') then
            spec_pd_min := '0'
          else
            spec_pd_min := FieldByName('pd_min').AsString;
          if (FieldByName('pd_max').AsString = '') then
            spec_pd_max := '0'
          else
            spec_pd_max := FieldByName('pd_max').AsString;
          if (FieldByName('vent').AsString = '') then
            spec_vent := '0'
          else
            spec_vent := FieldByName('vent').AsString;
          if (FieldByName('vent_min').AsString = '') then
            spec_vent_min := '0'
          else
            spec_vent_min := FieldByName('vent_min').AsString;
          if (FieldByName('vent_max').AsString = '') then
            spec_vent_max := '0'
          else
            spec_vent_max := FieldByName('vent_max').AsString;
          if (FieldByName('hd').AsString = '') then
            spec_hd := '0'
          else
            spec_hd := FieldByName('hd').AsString;
          if (FieldByName('hd_min').AsString = '') then
            spec_hd_min := '0'
          else
            spec_hd_min := FieldByName('hd_min').AsString;
          if (FieldByName('hd_max').AsString = '') then
            spec_hd_max := '0'
          else
            spec_hd_max := FieldByName('hd_max').AsString;
      ...
      if (Edit3.Text <> FieldByName('Checksum').AsString) then
      begin
        ...
        begin
          ...
          if cfg.Locate('ITEMS', spec_pid) then
          begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while (recordnr > 0) and (teller > 0) do
        begin
          ...
          begin   // measurement not found
            ...
            MeasFormat := ':DATE, :SHIFT, :ORIGIN, :PH, :ITEMS, :JTH, ' + ':HDMAX, :HDMIN, :HDMEAN, :HDSD, :HDCV, :HDCPK, :HDBAD, :HDDIV, ' +
            ...
            SQL.Text := 'INSERT INTO cigaret(`date`, `shift`, `origin`, `product`, `bunching_m`, `packing_m`, `max_hd`, `min_hd`, ' +
              ...
              '`out_weight`, `div_weight`, `YPBH`) ' + 'VALUES (' + MeasFormat + ')';
            ...
            if (Trim(ctdata1.GetField('HDMEAN', recordnr)) <> '') then
            begin
            ...
            if (Trim(ctdata1.GetField('CIRMEAN', recordnr)) <> '') then
            begin
            ...
            if (Trim(ctdata1.GetField('LENMEAN', recordnr)) <> '') then
            begin
            ...
            if (Trim(ctdata1.GetField('PDOMEAN', recordnr)) <> '') then
            begin
            ...
            if (Trim(ctdata1.GetField('VMEAN', recordnr)) <> '') then
            begin
            ...
            if (Trim(ctdata1.GetField('WGMEAN', recordnr)) <> '') then
            begin
          ...
          // loop de details af
          detailnr := 1;
          while detailnr < ctdata2.numrecs do
          begin
            if (Edit2.Caption + '-' + ctdata2.GetField('YPBH', detailnr)) = YPBH then
            begin  // check only the details of the current master
              nr := ctdata2.GetField('NO', detailnr);
              if Pos(nr, sticks) = 0 then
              begin  // store the stick that was not found on the server
                // create a new row
...
begin
  if Visible then
  begin // Application is visible, so minimize it to TrayIcon
    Application.Minimize; // This is to minimize the whole application
  end
  else
  begin // Application is not visible, so show it
